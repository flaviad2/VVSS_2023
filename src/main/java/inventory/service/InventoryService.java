...
public class InventoryService {
    ...
    public List<Part> filterPart(int min, int max) throws Exception{
        ArrayList<Part> all = new ArrayList<>();
        ...
        if(max < min) {
            return all;}
        for(Part p: repo.getAllParts()) {
            if(min==max) {
                continue;}
            if(p.getMin() >= min && p.getMax() <= max) {all.add(p);}
        }
        return all;
